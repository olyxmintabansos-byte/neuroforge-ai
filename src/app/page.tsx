...
export default function NeuroForgeStudioPage() {
  ...
  const [activeTab, setActiveTab] = useState<"ORCHESTRATOR" | "DEBATE_ARENA" | "ANALYTICS">("ORCHESTRATOR");
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6 font-sans pb-24">
      {/* Studio Navigation Tabs & Export Button */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-3 font-mono text-xs">
        <div className="flex items-center gap-1.5 bg-[#090f1d] p-1 rounded-2xl border border-slate-800">
          <button
          ...
          <button
            onClick={() => setActiveTab("DEBATE_ARENA")}
            className={`px-4 py-2 rounded-xl font-bold flex items-center gap-2 transition-all cursor-pointer ${
              activeTab === "DEBATE_ARENA"
                ? "bg-purple-500 text-white shadow-md shadow-purple-500/20"
                : "text-slate-400 hover:text-white"
            }`}
          >
            <Swords className="w-4 h-4" />
            <span>Debate Arena (Adversarial)</span>
          </button>
          ...
          >
            ...
            <span>Token ROI Telemetry</span>
          </button>
        </div>

        <button
          onClick={() => setIsExportOpen(true)}
          className="px-4 py-2 rounded-xl bg-[#0e162b] hover:bg-slate-800 border border-slate-700 hover:border-cyan-500/50 text-slate-200 hover:text-white font-bold flex items-center gap-2 transition-all cursor-pointer self-start sm:self-auto"
        >
          <FileDown className="w-4 h-4 text-cyan-400" />
          <span>Ekspor Specification (.md)</span>
        </button>
      </div>

      {/* TAB 1: AUTONOMOUS ORCHESTRATOR */}
      {activeTab === "ORCHESTRATOR" && (
        <div className="space-y-8 animate-in fade-in duration-150">
          ...
          <section className="bg-gradient-to-r from-[#070d1e] via-[#0f172a] to-[#070d1e] border border-cyan-500/30 rounded-3xl p-6 sm:p-8 relative overflow-hidden neon-glow-cyan">
            <div className="max-w-3xl space-y-4 relative z-10">
              ...
              <div className="pt-2">
                <span className="text-[11px] font-mono text-[#94a3b8] block mb-2 font-bold">Preset Misi Kelas Berat:</span>
